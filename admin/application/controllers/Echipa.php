...
class Echipa extends CI_Controller {
  ...
  public function index()
  {
		...
		//breacrumb
		$breadcrumb = array("bb_titlu" => "Echipa", "bb_button" => null, "breadcrumb" => array());
		$breadcrumb["bb_button"] = array("text" => '<i class="fa fa-plus-square"></i> Adauga membru', "linkhref" => $this->controller_fake ."/item/i");
		
		$breadcrumb["breadcrumb"][0] = array("text" => "Administrare", "url" => '/');
		$breadcrumb["breadcrumb"][1] = array("text" => "Echipa", "url" => null);
    $view = (object) [ 'html' => array(
			0 => (object) ["viewhtml" => "layout/breadcrumb", "viewdata" => $breadcrumb],
      1 => (object) ["viewhtml" => "echipa/index", "viewdata" => $viewdata]
      ), 'javascript' => array(
      1 => (object) ["viewhtml" => "echipa/js_index", "viewdata" => null],
      )
    ];
		...
		switch($this->uriseg->item)
    {
      ...
      case INSERT:
        ...
        if(isset($_REQUEST["{$form_submititem}"])) {
					...
					if($insert) {
						$this->_Session->setFB_Pozitive(array("ref" => "Servicii", "text" => "Ai adaugat un Membru in Echipa!"));
      ...
      break;
			...
			case DELETE:
				if(isset($this->uriseg->id) && !is_null($this->uriseg->id))
				{
					...
					$this->_Session->setFB_Pozitive(array("ref" => $this->Air->air_identplural, "text" => 'Ai sters un Membru!'));
		...
		$breadcrumb["breadcrumb"][0] = array("text" => "Echipa", "url" => $this->controller_fake);
		$breadcrumb["breadcrumb"][1] = array("text" => "Echipa dd", "url" => null);
    $view = (object) [ 'html' => array(
			0 => (object) ["viewhtml" => "layout/breadcrumb", "viewdata" => $breadcrumb],
      1 => (object) ["viewhtml" => "echipa/item", "viewdata" => $viewdata]
      ), 'javascript' => array(
      1 => (object) ["viewhtml" => "echipa/js_item", "viewdata" => null],
      )
    ];
    $this->frontend->render($view);	
	}
				...
				case UPLOADIMG:
					...
					switch($this->input->post("filetarget"))
					{
						case UPIMGPOZA:
							...
							$locations->path = '../web/' .PATH_IMG_ECHIPA;
