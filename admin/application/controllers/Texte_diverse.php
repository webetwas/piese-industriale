...
class Texte_diverse extends CI_Controller {
	...
	public function atom()
	{
		...
		(
			...
			"uri" => $this->uriseg,
			"form" => (object) [],
			"cp_main_nodes" => null // catalog produse main nodes
		);
		...
		switch($this->uriseg->atom)
		{
		  case UPDATE:
			if(isset($this->uriseg->id) && !is_null($this->uriseg->id))
			{
				...
				if($cp_main_nodes = $this->_Nodes->get_all_nodes_by_parent_id(1))
				{
					$viewdata["cp_main_nodes"] = $cp_main_nodes;
				}
				
				// Nodes
				if(!is_null($this->Air->targeted_nodes))
				{
					$nodes = $this->_Nodes->get_all_nodes_by_conditions(json_decode($this->Air->targeted_nodes));
				}
				else
				{
					$nodes = $this->_Nodes->get_all_nodes();
				}
				if($nodes) $viewdata["nodes"] = $nodes;
