...
class Item_model extends CI_Model {
          ...
          elseif(empty($this->input->post("{$input}")) && $Design["IS_NULLABLE"] == "YES")//set NULL value
            $insertArray[$column] = null;
					elseif(empty($this->input->post("{$input}")) && $Design["COLUMN_DEFAULT"] != null)//set Default value
						$insertArray[$column] = $Design["COLUMN_DEFAULT"];
        }
      }
    }
   ...
   * @return [type]        [description]
   */
  public function msqlGetAll($table, $data = null, $orderby = null)
	{
		if(!is_null($data))
		foreach($data as $keyd => $d)
			$this->db->where($keyd, $d);
	...
	public function msqlGetAllCereri($table)
	{
		$sql =
		'
			SELECT
				cereri.*,
				DATE_FORMAT(cereri.date, "%d.%m.%Y") as date,
				produs.*
			FROM
				' .$table. ' AS cereri LEFT JOIN
				catalog_produse AS produs ON cereri.atom_id = produs.atom_id
			GROUP BY
				cereri.cerere_id
			;		
		';
  ...
  public function QuestionTrueFalse($table, $column, $atom_id)
  {
    $res = array("status" => 0, "value" => null);

    $query = $this->db->query("SELECT " .$column. " FROM `" .$table. "` WHERE atom_id = " .trim(intval($atom_id)). ";");
    if($query->num_rows() > 0) {
      if($query->row_array()[$column]) {
        $querychange = $this->db->query("UPDATE `" .$table. "` SET `" .trim($column). "` = 0 WHERE atom_id = " .trim(intval($atom_id)). ";");
        $value = 0;
      } else {
        $querychange = $this->db->query("UPDATE `" .$table. "` SET `" .trim($column). "` = 1 WHERE atom_id = " .trim(intval($atom_id)). ";");
        $value = 1;
      }
      if($this->db->affected_rows() > 0) {
        $res["status"] = 1;
