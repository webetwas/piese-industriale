...
class Chain_model extends CI_Model
{
  ...
  public function __construct()
  {
	 ...
	 * Total of 1children
	 */
	public function getChildrens($id_parent)
	{
		$childrens = $this->msqlGetAll($this->tbl_chain_links, array("id_parent" => $id_parent));
		if($childrens) {
   ...
   * getChain
   */
  public function getChain($id_link)
  {
    
		$chain = $this->msqlGet($this->tbl_chain_links, array("id_link" => $id_link));
		if($chain) {
  ...
  /**
   * getLinksByConditions
	 *
	 * params @array
	 *
   */
	public function getLinksByConditions($schema)
	{
	
		$this->db->or_where_in($schema->column, $schema->values);
		$query = $this->db->get($this->tbl_chain_links);
	...
	{
		$sql = "SELECT
			{$this->tbl_obj_content}.idcontent_object,
			{$this->tbl_obj_content}.id_object,
			{$this->tbl_obj_content}.id_item,
			{$this->tbl_obj_content}.id_link,
			{$this->tbl_chain_links}.denumire_ro
   ...
   * getParentOfThis
   */
	public function getParentOfThis($id_link)
	{
		$category = $this->msqlGet($this->tbl_chain_links, array("id_link" => $id_link));
		if($category) {
      
      $root = $this->msqlGet($this->tbl_chain_links, array("id_link" => $category->id_parent));
      if($root)
        return $root;
    }
   ...
   * getAllLinksByParent
   */	
	public function getAllLinksByParent($id_parent, $any_condition = null)
	{
		$conditions = array("id_parent" => $id_parent);
		
		if(!is_null($any_condition))
			$conditions = array_merge($any_condition, $conditions);
		
		$links = $this->msqlGetAllOrderBy_chainorder($this->tbl_chain_links, $conditions);
		if($links) return $links;
   ...
   * MapAllLinks
   */
  function MapAllLinks($onlyfuckinggod = null)
	{
    $strs_a = $this->getAllLinksByParent(0, $onlyfuckinggod);
		if($strs_a)
   ...
   * @return [type]        [description]
   */
  public function msqlGetAllOrderBy_chainorder($table, $data = null)
	{
		if(!is_null($data))
		foreach($data as $keyd => $d)
			$this->db->where($keyd, $d);
      
    $this->db->order_by('chainorder', 'ASC');
